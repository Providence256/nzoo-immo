...
export class ListAnnoncesComponent implements OnInit {
  ...
  deleteSelectedAnnonces(): void {
    if (this.selectedAnnonces.length === 0) {
      this.messageService.add({
        severity: 'warn',
        summary: 'Attention',
        detail: 'Aucune annonce sélectionnée',
      });
      return;
    }
    ...
    if (
      confirm(
        `Êtes-vous sûr de vouloir supprimer ${this.selectedAnnonces.length} annonce(s)?`
      )
    ) {
      let successCount = 0;
      let errorCount = 0;
          ...
          next: () => {
            ...
            if (index === this.selectedAnnonces.length - 1) {
              this.messageService.add({
                severity: 'success',
                summary: 'Succès',
                detail: `${successCount} annonce(s) supprimée(s) avec succès${
          ...
          error: () => {
            ...
            if (index === this.selectedAnnonces.length - 1) {
              this.messageService.add({
                severity: 'error',
                summary: 'Erreur',
                detail: `${errorCount} erreur(s) lors de la suppression${
                  successCount > 0 ? `, ${successCount} succès` : ''
                }`,
              });
              this.selectedAnnonces = [];
              this.loadAnnonces();
  ...
  nextImage(): void {
    if (
      this.selectedAnnonceDetails &&
      this.carouselIndex < this.selectedAnnonceDetails.photoUrls.length - 1
    ) {
      this.carouselIndex++;
    }
  }
